...
void fill_numbase_buff(unsigned int num, unsigned base, char *buff, int buff_size);

/**
 * get_int - gets a character pointer to new string containing int
 * @num: number to convert to string
 *
 * Return: character pointer to newly created string (NULL)
 */
...
/**
 * get_numbase_len - fills buffer with correct numbers up to base 36
 * @num: integer to get absolute value
...
void fill_numbase_buff(unsigned int num, unsigned base, char *buff, int buff_size)
{
	...
	while (i >= 0)
	{
		...
		if (rem > 9) /* return lowercase ascii val representation */
			buff[1] = rem + 87; /* 10 will be a, 11 = b, ... */
		else
			buff[i] = rem + '0'
		num /= base;
		i--;
